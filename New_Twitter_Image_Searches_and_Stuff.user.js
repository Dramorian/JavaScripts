...
function UpdateDownloadControls() {
    ...
    let downloads_enabled = JSPLib.storage.getLocalData('ntisas-download-menu', {default_val: true});
...
function PageNavigation(pagetype) {
    ...
    if (IsPageType(STREAMING_PAGES) || IsTweetPage()) {
        ...
        if (IsPageType(['main', 'media', 'likes', 'replies'])) {
            if (NTISAS.user_settings.display_user_id || NTISAS.user_settings.display_profile_views) {
